...
struct sw_send { //struct powersw sensor;
...
struct sw_receive { //struct powersw sensor;
	char	ID[5];	//ИД устойства
	boolean	sw;		//состояние реле
};
sw_receive receive_data;
...
void loop(void) {
	...
	while (network.available()) {
		...
		if (String(receive_data.ID) == send_data.ID) {
			Serial.print("ID: ");
	...
	if ( now - last_time_sent >= interval ){
		...
		RF24NetworkHeader header( main_node, pws_header);
