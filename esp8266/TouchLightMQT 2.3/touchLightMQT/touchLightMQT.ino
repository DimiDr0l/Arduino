...
#define REPORT_INTERVAL 10 // отправка данных каждые 10 сек

#define pinLed1 4 //pwm Led1
#define pinLed2 5 //pwm Led2
#define pinOut 2 //out pin 9
...
const char* appass = "YOUR_AP_PASSWORD";
		...
		if (WiFi.status() == WL_CONNECTED) {
			if (!client.connected()) {
				...
				if (client.connect(deviceID.c_str(), eloginmqt.c_str(), epassmqt.c_str())) {
					...
					Serial.println(" try again in 1 seconds");  //Wait 5 seconds before retrying
					delay(1000);
					retries ++;
					if (retries == 600) {  //1 hour
						setupAP();
						return;
					}
				}
...
void dRead() {
	...
	if (!digitalRead(pinOut)) {
		key = readKey();
		if (!onStateAll && (currentMillis >= (timer1 + 70))) { //70мс на удержание вкл
			AllOn();
			return;
		}
		else if (onStateAll) {
			if ((key == 134 && !touchOn) || (key >= 1 && key <= 126 && !touchOn)) {  //кнопка выкл
				touchOn = true;
			} else if ((key != 134 && touchOn) || (key > 126 && touchOn)) {
				AllOff();
				touchOn = false;
			}
...
void AllOn() {
	for (int16_t i = 0; i <= _max(pwmLed1, pwmLed2); i++) {
		if (i <= pwmLed1) analogWrite (pinLed1, i);
		if (i <= pwmLed2) analogWrite (pinLed2, i);
		delay (delayDimmer);
	}
	stat[0] = stat1;
...
void AllOff() {
	for (int16_t i = 0; i <= _max (pwmLed1, pwmLed2); i++) {
		if (pwmLed1 - i >= 0) analogWrite (pinLed1, pwmLed1 - i);
